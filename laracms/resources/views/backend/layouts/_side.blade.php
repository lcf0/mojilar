                            ...
                            <?php if (empty($item['children'])): ?>
                                ...
                                <?php else: ?>
                                   <ul class="sidebar-menu" >
                                         <li class="treeview @if($activeNavId == $item['id']) active @endif" id="2">
                                             <a href="@if(!empty($item['link'])) {{$item['link']}} @elseif(!empty($item['route'])) {{route($item['route'], $item['params'])}}@if(!empty($item['query']))?{{implode('&',$item['query'])}}@endif @else javascript:; @endif">
                                                <i class="icon {{ empty($item['icon']) ? 'icon-circle-blank' : $item['icon'] }}"></i>
                                                <span>{{ $item['text'] }}</span>
                                                <span class="pull-right-container">
                                                    <i class="icon icon-angle-left"></i>
                                                </span>
                                            ...
                                            <ul class="treeview-menu">
                                                @foreach($item['children'] as $key3 => $val)
                                                @if(call_user_func($val['permission']))
                                                    <li id="nav_{{$key2}}_{{$key3}}" class="@if($activeNavId == $val['id']) active @endif">
                                                        <a href="@if(!empty($val['link'])){{$val['link']}}@elseif(!empty($val['route'])){{route($val['route'], $val['params'])}}@if(!empty($val['query']))?{{implode('&',$val['query'])}}@endif @else javascript:;@endif">
                                                            <i class="icon {{ empty($val['icon']) ? 'icon-circle-blank' : $val['icon'] }}"></i> {{ $val['text'] }}
                                                        </a>
                                                    </li>
                                                @endif
                                            @endforeach
