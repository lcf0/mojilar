...
.div_check{
                        ...
                        <div class="form-group has-feedback has-icon-right">
                            ...
                            <!-- <div class="checkbox"> -->
                              ...
                              <?php foreach ($data as $key => $v) {
                                 $str = "<div style='' class = 'maxbox'>
                                        <fieldset>
                                            <legend><input name='permission[]' class = 'minbox' type='checkbox' value='".$v['name']."' title='".$v['remarks']."'  ";
                                            if (in_array($v['name'], $rolePermissions)) {
                                                $str.='checked="checked"';
                                            }
                                            $str.=" required>".$v['remarks']."</legend>";
                                            if (@is_array($v['child'])) {
                                               for ($i=0; $i < count($v['child']); $i++) {
                                                $str.="<div class='div_check' >";
                                                   $str.= "<input name='permission[]' class='check_box' type='checkbox' value='".$v['child'][$i]['name']."' title='".$v['child'][$i]['remarks']."' ";
                                                   if (in_array($v['child'][$i]['name'], $rolePermissions)) {
                                                        $str.='checked="checked"';
                                                    }
                                                   $str.=" />".$v['child'][$i]['remarks']. " </div>";
                                               }
                                            }
    ...
    <script type="text/javascript">
       $(document).on("click",".check_box",function(){
        $(this).parents(".maxbox").children().find('.minbox').prop('checked',true);
       })
       $('.minbox').change(function(){
           $(this).parents('.maxbox').children().find('.check_box').prop('checked',$(this).is(':checked')?true:false);
        });
    </script>
@endsection
